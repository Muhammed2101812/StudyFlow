...
require$$1$2.ipcMain.handle("read-file", async (event, filePath) => {
  try {
...
require$$1$2.ipcMain.handle("write-file", async (event, filePath, content) => {
  try {
...
require$$1$2.ipcMain.handle("open-file-dialog", async () => {
  try {
    const result = await require$$1$2.dialog.showOpenDialog(mainWindow, {
      properties: ["openFile"],
...
require$$1$2.ipcMain.handle("save-file-dialog", async (event, defaultPath) => {
  try {
    const result = await require$$1$2.dialog.showSaveDialog(mainWindow, {
      defaultPath,
      filters: [{ name: "JSON Files", extensions: ["json"] }]
    });
    return { success: true, data: result };
...
require$$1$2.ipcMain.handle("store-get", async (event, key) => {
  try {
    return store.get(key);
  } catch (error2) {
    console.error("Store get error:", error2);
    return void 0;
  }
});
require$$1$2.ipcMain.handle("store-set", async (event, key, value) => {
  try {
    store.set(key, value);
    return true;
  } catch (error2) {
    console.error("Store set error:", error2);
    return false;
  }
});
require$$1$2.ipcMain.handle("store-delete", async (event, key) => {
  try {
    store.delete(key);
    return true;
  } catch (error2) {
    console.error("Store delete error:", error2);
    return false;
  }
});
require$$1$2.ipcMain.handle("store-has", async (event, key) => {
  try {
    return store.has(key);
  } catch (error2) {
    console.error("Store has error:", error2);
    return false;
  }
});
require$$1$2.ipcMain.handle("store-clear", async () => {
  try {
    store.clear();
    return true;
  } catch (error2) {
    console.error("Store clear error:", error2);
    return false;
  }
});
require$$1$2.ipcMain.handle("store-get-all", async () => {
  try {
    return store.store;
  } catch (error2) {
    console.error("Store get-all error:", error2);
    return {};
  }
});
