import electron, { ipcMain as ipcMain$1, app as app$1, Menu, BrowserWindow } from "electron";
...
const storage = {
  ...
  getUserData: (userId, dataType2) => store.get(`users.${userId}.${dataType2}`),
  setUserData: (userId, dataType2, data) => {
    store.set(`users.${userId}.${dataType2}`, data);
  },
  initializeDefaults: () => {
    if (!store.has("users")) {
      store.set("users", []);
...
function createWindow() {
  ...
  const preloadPath = fs.existsSync(preloadMjs) ? preloadMjs : fs.existsSync(preloadJs) ? preloadJs : preloadMjs;
...
const menuTemplate = [
  {
    ...
    submenu: [
      ...
      { type: "separator" },
      { label: "Quit", accelerator: "Command+Q", click: () => app$1.quit() }
    ]
  },
  {
    label: "Edit",
...
ipcMain$1.handle("read-file", async (event, filePath) => {
  try {
...
ipcMain$1.handle("save-file", async (event, data, defaultPath) => {
  try {
...
ipcMain$1.handle("store-get", async (event, key) => {
  return storage.get(key);
});
ipcMain$1.handle("store-set", async (event, key, value) => {
  storage.set(key, value);
  return true;
});
ipcMain$1.handle("store-delete", async (event, key) => {
  storage.delete(key);
  return true;
});
ipcMain$1.on("store-get-sync", (event, key) => {
  event.returnValue = storage.get(key);
});
ipcMain$1.on("store-set-sync", (event, key, value) => {
  storage.set(key, value);
  event.returnValue = true;
});
ipcMain$1.on("store-delete-sync", (event, key) => {
  storage.delete(key);
  event.returnValue = true;
});
ipcMain$1.handle("path-join", async (event, ...paths) => {
